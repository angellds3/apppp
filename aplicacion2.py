...
nsimulaciones = st.number_input("Número de simulaciones", min_value=1, value=1000, step=1)
nreplicas = st.number_input("Número de réplicas", min_value=1, value=10, step=1)
ndias = st.number_input("Número de días", min_value=1, value=30, step=1)
exito = st.slider("Probabilidad de éxito (%)", min_value=0, max_value=100, value=50) / 100
fecha_inicio = st.date_input("Fecha de inicio")
capital_inicial = st.number_input("Capital inicial ($)", min_value=0.0, value=1000.0, step=10.0)
pip_win = st.number_input("Pips por operación ganadora", value=20.0, step=1.0)
pip_lose = st.number_input("Pips por operación perdedora", value=-15.0, step=1.0)
valor_pip = st.number_input("Valor de un pip ($)", min_value=0.01, value=1.0, step=0.01)
criterio_fracaso = st.number_input("Criterio de fracaso (capital mínimo $)", min_value=0.0, value=500.0, step=10.0)
criterio_exito = st.number_input("Criterio de éxito (capital máximo $)", min_value=0.0, value=1500.0, step=10.0)
...
if st.button("Iniciar Simulación"):
  ...
  st.write(f"**Simulaciones con éxito:** {simulacion_exito} (capital final > {criterio_exito})")
  st.write(f"**Simulaciones con fracaso:** {simulacion_fracaso} (capital final < {criterio_fracaso})")
  st.write(f"")
